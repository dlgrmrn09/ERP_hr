...
export const listUsers = asyncHandler(async (req, res) => {
  ...
  const sortField = mapSortField(typeof sort === "string" ? sort : undefined);
...
interface RoleIdResult {
  role_id: number;
}

const fetchRoleId = async (roleName: string): Promise<number | undefined> => {
  const roleRes = await pool.query<RoleIdResult>(
...
export const createUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json({ user: { ...userResult.rows[0], role: roleName } });
});
...
const ensureMoreThanOneAdmin = async (
  client: PoolClient,
  excludingUserId: number
): Promise<boolean> => {
  const result = await client.query<AdminCountResult>(
